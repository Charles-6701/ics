...
void read_phase(char filename[])
{
    ...
    phase_text[0] = '\0';  // 初始化 phase_text

    // 安全地读取文件内容
    while (fread(&tmp, sizeof(__uint8_t), 1, fp) == 1) {
        sprintf(tmp_txt, "%02x", tmp);
        strncat(phase_text, tmp_txt, sizeof(phase_text) - strlen(phase_text) - 1); // 使用 strncat 确保不会溢出
    }
